...
MAINTAINER Javier Fernandez <javier.fernandez.pastrana@gmail.com>                               
...
RUN mkdir -p ${YOCTO_INSTALL_PATH}/gco-bsp && cd ${YOCTO_INSTALL_PATH}/gco-bsp && repo init -u https://github.com/gcoUVa/v2x-platform.git -b ${YOCTO_RELEASE} && repo sync 
                                                                                 
# Create a build directory for the FSL community BSP                             
RUN mkdir -p ${YOCTO_INSTALL_PATH}/gco-bsp/build                       
